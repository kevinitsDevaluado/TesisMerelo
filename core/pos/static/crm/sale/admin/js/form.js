...
var input_nameProduct;
var input_priceProduct;
var input_categoryProduct;
...
var input_search_products_name;
var input_endcredit;
var inputs_vents;
    ...
    fvSale = FormValidation.formValidation(frmSale, {
        ...
        .on('core.form.valid', function () {
            ...
            if (vents.details.products.length === 0) {
                message_error('Registro de Inicio correcto.!');
...
document.addEventListener('DOMContentLoaded', function (e) {
    ...
    let fvProduct = FormValidation.formValidation(frmProduct, {
            locale: 'es_ES',
            localization: FormValidation.locales.es_ES,
            plugins: {
                trigger: new FormValidation.plugins.Trigger(),
...
$(function () {
    ...
    input_search_products_name = $('input[name="search_products_name"]');
    ...
    input_nameProduct = $('input[name="name"]');
    input_priceProduct = $('input[name="pvp"]');
    input_categoryProduct = $('select[name="category"]');
    ...
    input_search_products_name.autocomplete({
        source: function (request, response) {
            $.ajax({
                url: pathname,
                data: {
                    'action': 'search_products_name',
                    'term': request.term,
                    'ids': JSON.stringify(vents.get_products_ids()),
                },
                dataType: "json",
    ...
    $('.btnAddAllProducts').on('click', function () {
        //input_birthdate.datetimepicker('date', new Date());
